...
from heylux.pulse import breathing_pulse, _get_alert_lights
...
class TestAlertLightsConfig:
    def test_defaults_to_all_lights(self, mock_bridge):
        with patch("heylux.pulse.Path") as mock_path:
            mock_path.home.return_value = MagicMock()
            config_file = mock_path.home() / ".config" / "heylux" / "calendars.json"
    ...
    def test_reads_config(self, tmp_path, mock_bridge):
        ...
        with patch("heylux.pulse.Path.home", return_value=tmp_path):
            # Need to construct the right path
            (tmp_path / ".config" / "heylux").mkdir(parents=True, exist_ok=True)
            (tmp_path / ".config" / "heylux" / "calendars.json").write_text(
                json.dumps(config)
            )
            lights = _get_alert_lights()
